...
f. **Reformat overcrowded slides** — If a slide has too much content, or two figures stacked one above the other (or side by side), split it into two slides. If text is too small to read, reformat or regenerate the slide for legibility.
...
h. **Apply formatting** — Enforce STYLE_GUIDE.md on every slide of the deck: fonts, colors, title position/formatting, backgrounds per slide type, footer, margins, line spacing, max bullets per slide. This applies to ALL slides even if it means restyling or regenerating existing ones.
